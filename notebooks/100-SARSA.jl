...
# ╔═╡ 77ea5d27-39b9-4555-b588-a4ad6a178edf
using Plots

# ╔═╡ e7c8a6b4-96d4-4a06-89be-a2ef527b9e19
md"""
# SARSA
"""

# ╔═╡ 354c34ba-441f-4e56-8721-90fa1ee7e46b
...
# ╔═╡ f451c68d-5956-4737-96f2-4d3bfbf66d18
...
# ╔═╡ a5eedf98-3665-42f8-95ea-35470cc2e7e2
begin
	m = SimpleGridWorld()
...
sarsa_episodes = sarsa!(env, n_episodes=10_000, α=0.2);
...
# ╔═╡ 004796bb-5c13-47d2-bfb7-592a77b99e2f
...
# ╔═╡ 001c5f40-6d07-4f40-aada-2ce1d1f2b76b
...
lambda_episodes = sarsa_lambda!(env, n_episodes=10000, α=0.1, λ=0.7);
...
begin
	...
	if step < length(ep.hist)
	 	if episode > 1
	 		prev_ep = sarsa_episodes[episode-1]
			POMDPTools.render(m, (s=ep.hist[i],), color=s->maximum(map(a->prev_ep.Q[(s,a)], actions(env))))
		else
			POMDPTools.render(m, (s=ep.hist[i],), color=s->0.0)
		end
	else
		POMDPTools.render(m, (s=ep.hist[i],), color=s->maximum(map(a->ep.Q[(s,a)], actions(env))))
	end
...
begin
	...
	if step < length(l_ep.hist)
	 	if episode > 1
	 		l_prev_ep = lambda_episodes[episode-1]
			POMDPTools.render(m, (s=l_ep.hist[l_i],), color=s->maximum(map(a->l_prev_ep.Q[(s,a)], actions(env))))
		else
			POMDPTools.render(m, (s=l_ep.hist[l_i],), color=s->0.0)
		end
	else
		POMDPTools.render(m, (s=l_ep.hist[l_i],), color=s->maximum(map(a->l_ep.Q[(s,a)], actions(env))))
	end
...
# ╔═╡ f536cb24-3641-4b3d-af1a-3d40f3710703
...
# ╔═╡ 38d2bbe2-c17a-4799-83c6-cc15b0b2d720
episodes = Dict("SARSA"=>sarsa_episodes, "SARSA-λ"=>lambda_episodes)

# ╔═╡ beaa1e25-4a19-42da-bd9d-958fbe34df5d
md"""
## By # Steps
...
# ╔═╡ 0256dc40-b91d-4942-99a1-fd6cafa58dff
function learning_curve_steps(episodes)
...
# ╔═╡ 55672a82-c2be-4acd-a33e-c10c91c0967e
learning_curve_steps(episodes)

# ╔═╡ d8234d45-3dc5-440b-9930-c1b86468a900
md"""
## By wall clock time
"""

# ╔═╡ 167d6bea-e1dc-4e14-aee0-5f36cdd3dd72
function learning_curve_clock(episodes)
...
manifest_format = "2.0"
project_hash = "a9e0502f8a5e0de46a543caca9eb72b949c1d2f4"
