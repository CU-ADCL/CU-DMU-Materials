...
\begin{question} (10 pts) Do similar $Q$ values imply similar rewards?
    ...
    {\color{purple}Let $\beta$ be a positive constant. If a policy $\pi$ satisfies $|Q^*(s, \pi^*(s)) - Q^*(s, \pi(s))| \leq \beta$ for all $s \in S$, then it immediately follows that $|R(s, \pi^*(s)) - R(s, \pi(s))| \leq \beta$ for all $s \in S$.}
