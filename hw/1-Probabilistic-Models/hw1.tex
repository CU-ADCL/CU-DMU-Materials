...
\begin{itemize}[nosep]
    ...
    \item JSON file output from \texttt{DMUStudent.HW1.evaluate}.
...
\begin{question} (40 pts)
    Consider the following Bayesian network structure where all variables are binary:
    ...
    \begin{tikzpicture}
        ...
        \path [->,line width=1pt] (D) edge node[left]{}(C);
        \path [->,line width=1pt] (C) edge node[left]{}(B);
        \path [->,line width=1pt] (C) edge node[left]{}(E);
        \path [->,line width=1pt] (B) edge node[left]{}(E);
    \end{tikzpicture}
    \end{center}

    \begin{enumerate}[nosep,label=\alph*)]
        \item Is it possible to conclude from the structure that $A \perp E \mid C$? Justify your answer.
        \item Is it possible to conclude from the structure that $A \perp D \mid C$? Justify your answer.
        \item Suppose that $P(E=0 \mid A=0, C=1, B=1) = 0.3$. What is $P(E=1 \mid A=1, C=1, B=1)$? Justify your answer.
        \item Assume that $P(B=1 \mid C=0) = 0.6$, $P(E=0 \mid B=1, C=0) = 0.1)$ and $P(E=0 \mid B=0, C=0) = 0.8$. If we observe that $C=0$ and $E=0$, what is the probability that $B=1$? Justify your answer.
    \end{enumerate}
\end{question}
