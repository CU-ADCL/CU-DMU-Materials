...
\begin{question}(25 points)
    Jill is an entrepreneur of a small business trying to decide whether to commit to sourcing inputs for her business from a domestic supplier or an international supplier. The legislature of her country has delegated some tariff authority to the executive branch, but the precise legal limits are unclear, and the president has imposed tariffs with dubious legality.

    This results in a POMDP for Jill. The state space is $S = \{T, F\}$, where $T$ corresponds to the tariffs being legal and upheld and $F$ indicates that the tariffs are illegal and free trade will be restored. The action space is $A = \{D, I\}$, where $D$ indicates that Jill commits to a domestic supplier and $I$ indicates that she commits to an international supplier.
...
\begin{enumerate}[label=\alph*)]
    \item Draw or plot the one-step alpha vectors for the one-step plans corresponding to the two actions in the manner done in class. Label each alpha vector with its corresponding action and indicate the rewards for each state on the sides of the plot as done in class.
    \item If Jill's believes that tariffs will be found legal with 60\% likelihood, what is the optimal action if the problem only has one step? Justify your answer numerically.
    \item What is the expected reward taking the optimal action given her belief?
\end{enumerate}

Now assume that there is a third action $W$, which corresponds to waiting for the courts to determine the legality. The wait action has an immediate reward of -1 for any state. After a wait action is taken, a perfectly accurate observation from $O = \{T, F\}$, which reveals the true state, is received.
...
\begin{enumerate}[label=\alph*),resume]
    ...
    \item Calculate the alpha vector for this conditional plan and draw or plot it on the same axes as the one-step alpha vectors from part (a). \footnote{Drawing the one-step and two step alpha vectors on the same axes is equivalent to saying that problem is infinite-horizon, but terminates after Jill takes a $T$ or $D$ commitment action.} Assume a discount factor of 1.
    \item If Jill's belief is uniform over the two states, which of the three alpha vectors corresponds to the optimal plan? Justify your answer numerically.
    \item What is the expected cumulative reward of executing the optimal plan given her belief? Show your calculations.
\end{enumerate}
