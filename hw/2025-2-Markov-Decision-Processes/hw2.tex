...
\begin{question}\label{q:finite_mdp} (25 pts)
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\linewidth]{hw/2025-2-Markov-Decision-Processes/problem2_image.png}
        \label{fig:placeholder}
    \end{figure}
    \begin{enumerate}[label=\alph*)]
        \item Formulate this problem as an MDP (write down $S$, $A$, $T$, and $R$).
        \item What is the optimal policy for this game, assuming a discount factor $\gamma = 0.95$?
    \end{enumerate}
\end{question}

\begin{question}\label{q:infinite_mdp} (25 pts)
    ...
    \begin{enumerate}[label=\alph*)]
        \item Formulate this problem as an MDP (write down $S$, $A$, $T$, and $R$).
        \item Evaluate the policy $\pi = [roll, reset, reset]$.
